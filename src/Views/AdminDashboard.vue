...
// Toggling the sidebar open/closed swaps which element is on screen
// (collapsed logo-button <-> expanded collapse-button). Since isLogoHovered
// is shared between both, it used to "leak" across the swap: click while
// hovering, and the brand-new element on the other side would render as
// already-hovered (wrong icon + tooltip stuck showing) until the mouse
// physically left and re-entered it. Resetting it on every toggle fixes that.
...
const completedBookingsCount = computed(() =>
  allBookings.value.filter((b) => b.booking_status === 'Completed').length
)
